...
const TELEMETRIA = false;
// Wysyła mi ten plik zadania na server.
const TELEMETRIA_ZAAWANSOWANA = false;
...
*/
odpowiedź("A");
uzasadnienie(`typy danych to takie coś jak number, boolean, string`);
...
*/
odpowiedź("const");
uzasadnienie(`const to stała którą nie można zmienić, tylko można zmodyfikować`);
...
*/
odpowiedź("undefined");
uzasadnienie(`nie rozumiem obiektów tak dobrze jeszcze wiec wpisuje moja 1 mysl: 
              dlatego ze f nie jest przypisane w obiekcie i kaze nam to wywołać obiekt o które jest w obiekcie f`);
...
*/
odpowiedź("B");
uzasadnienie(`B ponieważ pętla słuzy do powtarzania powtarzających się czynności`);
...
*/
odpowiedź("5");
uzasadnienie(`petla for pisze ze i rowna sie 5 i jest mniejsze niz 100 (nie mniejsze rób równe 100 czyli wartosc ktora osiagnie 100 nie zostanie uznana) 
              potem i jest mnozone przez razy 2 i jest kontynuowane az osiagnie 100 ale jak osiagnie 100 to nie wykona 100 dlatego ze limit jest ponizej 100 a nie rowny 100
              edit: jestem debilem i zapomnialem ze i jest przypisywane co wykonanie XD więc te i będzie na poczatku 5 potem 10 potem 20 potem 40 potem 80 i sie skonczy`);
...
*/
odpowiedź("1010");
uzasadnienie(`w javascript obowiązuje zasada left to right wiec w tym po lewej jest string a po prawej number ale zamienia na string ze wzgledu ze jest po lewej`);
...
*/
odpowiedź("4");
uzasadnienie(`nwm chce materiał poczytać bo nie umiem dobrze obiekty 
              edit: wywali błąd bo a nie jest nigdzie przypisane?
              edit 2: przeczytałem jeszcze raz artykuł i widze ze nie trzeba deklarować wczesniej i z obiektem samo sie deklaruje:
              w 1 linijce tworzymy stałą o i przypisujemy do niej że jest obiektem.
              w 2 linijce tworzymy i przypisujemy do obiektu o klucz a ktory zawiera liczbe 10
              w 3 linijce dodajemy stałą b którą przypisujemy ze jest obiektem i te 3 kropki z o sprawiaja ze wywolany zostaje cały obiekt o
              w 4 linijce przypisujemy do b a ktore jest rowne liczbie 10
              w 5 linijcie przypisujemy do obiektu b klucz b o wartosci 11
              edit 3: powiniem dostać wpierdol za niezrozumienie polecenia xd, i błąd raczej jest w linijce 3 bo mi mocno śmierdzi i przypisujesz do obiektu b klucz a ktory jest tylko dostepny poprzez wywołanie obiektu o`);
...
uzasadnienie(`stworzyłem funkcje zwykłą
              edit 1: nie umiem czytać i stworzyłem funkcje z parametrami number ktorych nie mozna xd`);
...
uzasadnienie(`odnosze sie do obiektu drzewo i po kropce do klucza kasztan w drzewie`);
...
uzasadnienie(`odwołuje się do 10 elementu tablicy kasztany
              edit 1: napisałeś element 10 bez uwzględnienia 0 aok`);
...
*/
odpowiedź("infinity");
uzasadnienie(`undefined bo jeśli dobrze pamiętam to pętla for kończyła się na wartości false gdzie false to tez 0
              edit 1: nie undefined bo ma to wartość za szybko to pisałem
              edit 2: aok jednak nie pamiętam dobrze`);
